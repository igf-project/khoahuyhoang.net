			...
			if($viewtype=="block") {
				...
				if($cat_name!='') {
					...
					for($i=0;$i<count($arr_name)-1;$i++) 
						$path_name.= '<li><a href="'.ROOTHOST.'tin-tuc/'.un_unicode($arr_name[$i]).'/">'.$arr_name[$i].'</a></li>';
				}
				...
				$path_name = '<li><a href="'.ROOTHOST.'">Tìm kiếm</a></li>';
			}
			break;
		case 'contact': 
			$path_name = '<li><a href="'.ROOTHOST.'lien-he/">Liên hệ</a></li>';
			break;
		case 'gallery': 
			$path_name = '<li><a href="'.ROOTHOST.'du-an/">Dự án</a></li>';
			if($viewtype=='block') {
				$id = isset($_GET['id'])?(int)($_GET['id']):0;
				$objga = new CLS_GALLERY;
				$ablum = $objga->getNameById($id); unset($objga);
				$path_name .= '<li><a href="'.ROOTHOST.'du-an/'.$id.'">'.$ablum.'</a></li>';
			}
			break;
		case 'products':
			$path_name = '<li><a href="'.ROOTHOST.'san-pham/">Sản phẩm</a></li>';
			$code = isset($_GET['code'])?addslashes($_GET['code']):'';	
			if($code!='') {
...
<div id="path"class="container-fluid">
	<div class='container'>
		<ul class="pull-left col-md-8 col-sm-6 col-xs-12">
		<li><a href="<?php echo ROOTHOST;?>" class="home">
		<img src="<?php echo ROOTHOST.'images/icons/glyphicons-21-home.png';?>" width="20" align="middle" alt="Khóa Huy Hoàng"/>
		</a></li>
		<?php echo $path_name;?>
		</ul>
		<?php if($viewtype=="detail" or $viewtype=="article") {?>
		<div class="pull_right like_share_box col-md-4 col-sm-6 col-xs-12">
