...
class CLS_CATE{
	...
	function listTableCate($strwhere="",$page=1,$parid=0,$level=0,$rowcount){
		...
		while($rows=$objdata->Fetch_Assoc()){	
			...
			$alias=$rows['alias'];
			$title=stripslashes($rows['meta_title']);
			echo "<tr name='trow'>";
			...
			echo "<td nowrap='nowrap'>$alias</td>";
			echo "<td nowrap='nowrap' title='$title'>".Substring($title,0,15)."&nbsp;</td>";
	...
	function Add_new(){
		...
		$sql=" INSERT INTO `tbl_category_text`(`cat_id`,`name`,`alias`,`meta_title`,`meta_key`,`meta_desc`,`lag_id`) VALUES";
		$sql.="('$cat_id','".$this->Name."','".$this->Alias."','".$this->MTitle."','".$this->MKey."','".$this->MDesc."','".$this->LangID."')";
	...
	function Update(){
		$sql = "UPDATE tbl_category SET par_id='".$this->ParID."',`isactive`='".$this->isActive."' WHERE cat_id='".$this->ID."'";
		$this->objmysql->Exec('BEGIN');
		$result=$this->objmysql->Exec($sql);
		
		$sql = "UPDATE `tbl_category_text` SET `lag_id`='".$this->LangID."',`name`='".$this->Name."',`alias`='".$this->Alias."',`meta_title`='".$this->MTitle."',`meta_key`='".$this->MKey."',`meta_desc`='".$this->MDesc."' ";
		$sql.=" WHERE `cat_id`='".$this->ID."'";
		//echo $sql; die();
		$result1=$this->objmysql->Exec($sql);
		if($result && $result1 ){
			$this->objmysql->Exec('COMMIT');
