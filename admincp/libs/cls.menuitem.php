...
class CLS_MENUITEM{
	...
	function listTableItemMenu($strwhere="",$page,$par_id,$level){
		$sql="SELECT * FROM `view_menuitem` WHERE `par_id`='$par_id' AND ".$strwhere." ORDER BY `order` ASC, mnuitem_id ASC";
		$objdata=new CLS_MYSQL;
		$objdata->Query($sql);
		$str_space="";
		if($level!=0){
	...
	public function Add_new(){
		$sql="INSERT INTO `tbl_mnuitem`(`par_id`,`code`,`mnu_id`,`viewtype`,`cat_id`,`con_id`,`cata_id`,`link`,`class`,`isactive`) VALUES ";
		$sql.=" ('".$this->Par_ID."','".$this->Code."','".$this->Mnu_ID."','".$this->Viewtype."','".$this->Cat_ID."','".$this->Con_ID."','".$this->Cata_ID."','".$this->Link."','".$this->Class."','".$this->isActive."') ";
		//echo $sql; die();
	...
	function Order($arr_id,$arr_quan){
		$n=count($arr_id);
		for($i=0;$i<$n;$i++){
			$sql="UPDATE `tbl_mnuitem` SET `order`='".$arr_quan[$i]."' WHERE `mnuitem_id` = '".$arr_id[$i]."' ";
