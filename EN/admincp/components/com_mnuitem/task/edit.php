...
function checkinput(){	
	...
	else if($("#cbo_viewtype").val()=="catalog"){
		if($("#cbo_cata").val()=="0") {
			$("#catalog_err").fadeTo(200,0.1,function()
			{ 
			  $(this).html('Vui lòng chọn một nhóm sản phẩm').fadeTo(900,1);
			});
			$("#cbo_cata").focus();
			return false;
		}
	  ...
	  <tr>
  	    ...
  	    <td>
  	      <select name="cbo_cata" id="cbo_cata">
  	        <option value="0" title="Top">Chọn nhóm SP</option>
           <?php
		  	if(!isset($objCata))
			$objCata=new CLS_CATALOGS();
			if ($obj->Cata_ID==0) $objCata->getListCate(0,0);
			else $objCata->getListCate($obj->Cata_ID,0);
		  	?>
	      </select> 
		  <script language="javascript">
				cbo_Selected('cbo_cata','<?php echo $row['cata_id'];?>');
            </script>
	      <label id="catalog_err" class="check_error"></label>
        </td>
      </tr>
      <?php } else { ?>
  	  <tr>
  ...
  <fieldset>
    ...
    <legend><strong><?php echo CDESC;?>:</strong></legend>
          <?php //Create_textare("txtdesc",'oEdit1');?>
            <textarea name="txtdesc" id="txtdesc" cols="45" rows="5"><?php echo stripslashes($row['intro']);?></textarea>
        	<script language="javascript">
