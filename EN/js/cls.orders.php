...
class CLS_ORDER{
	...
	public function Add_new(){
		...
		for($i=0;$i<$n;$i++){
			...
			if($_SESSION['CART'][$i]['color']=='') $color=$rs['thumb'];
			else $color=$_SESSION['CART'][$i]['color'];
			$sql.=" ('".$order_id."','".$_SESSION['CART'][$i]['code']."','".$_SESSION['CART'][$i]['quan']."','".$rs['cur_price']."','".$color."','".$_SESSION['CART'][$i]['size']."')";
			if($i<$n-1) $sql.=" , ";
		}
		echo $sql;
