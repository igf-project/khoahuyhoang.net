...
if($item==0) 
	...
	while($arr = $objcat->Fetch_Assoc()) {
		$name = substr($arr['name'],0,1).substr($arr['name'],1,strlen($arr['name']));		
		if($code!='') 
			echo '<h1 class="tieu-de"><a href="'.ROOTHOST.un_unicode($arr['name']).'" title="Group '.$name.'">Group '.$name.'</a></h1>';
		else { 
			if($i==1) echo '<h1 class="tieu-de"><a href="'.ROOTHOST.un_unicode($arr['name']).'" title="Group '.$name.'">Group '.$name.'</a></h1>';
			else echo '<h2 class="tieu-de"><a href="'.ROOTHOST.un_unicode($arr['name']).'" title="Group '.$name.'">Group '.$name.'</a></h2>';
			$i++;
		} ?>
	<div class="products">
		<?php 
			...
			if($code=='') {
				...
				echo '<div class="read-more">
					<a href="'.ROOTHOST.$arr['alias'].'">
						<span class="caret"></span> View all products groups "'.$arr['name'].'"
					</a>
				</div><br>';
			}else{ 
