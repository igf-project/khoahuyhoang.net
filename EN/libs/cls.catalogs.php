...
class CLS_CATALOGS{
			...
			while($row=$objdata->Fetch_Assoc()){
				...
				echo '<article class="catalog col-md-4 col-sm-4 col-xs-6 text-center"> 
						<div class="content-inner">
							<a href="'.$link.'" class="product-thumb" title="'.$row['name'].' Huy Hoang"><img src="'.$img.'" class="img-responsive" alt="'.$row['name'].'"/></a>
  ...
  '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600'".');return false;"></a>
			...
			echo '</div>';
			if($par_id==0)
				echo '<div class="read-more"><a href="'.ROOTHOST.'san-pham/'.'"><span class="caret"></span> View all products</a></div>';
		}
	}
